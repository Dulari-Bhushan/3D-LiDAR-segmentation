...
class VisionTransformer(nn.Module):
    ...
    def forward(self, im, return_features=False, return_attention_stats=False):
        B, _, H, W = im.shape
        ...
        for blk in self.blocks:
            if return_attention_stats:
                x, attn = blk(x, return_output_and_attention=True)
        ...
        if return_attention_stats:
            return x, skip, attn_maps  # x.shape = [16, 577, 384]
...
def create_rangevit(model_cfg, use_kpconv=False, use_geometry_fusion=False, geometry_fusion_channels=32, geometry_in_channels=4):
    model_cfg = model_cfg.copy()
    ...
    if use_kpconv:
        ...
        model = RangeViT_noKPConv(
            ...
            use_geometry_fusion=use_geometry_fusion,
            geometry_fusion_channels=geometry_fusion_channels,
            geometry_in_channels=geometry_in_channels,
        )
...
class RangeViT_noKPConv(nn.Module):
    def __init__(
        ...
        self.use_geometry_fusion = use_geometry_fusion and hasattr(self.decoder, 'd_decoder')
        self.geometry_in_channels = geometry_in_channels

        if self.use_geometry_fusion:
            d_decoder = self.decoder.d_decoder
            self.geometry_proj = nn.Sequential(
                nn.Conv2d(self.geometry_in_channels, geometry_fusion_channels, kernel_size=1),
                nn.LeakyReLU(),
                nn.BatchNorm2d(geometry_fusion_channels),
            )
            self.geometry_fusion = nn.Sequential(
                nn.Conv2d(d_decoder + geometry_fusion_channels, d_decoder, kernel_size=3, padding=1),
                nn.LeakyReLU(),
                nn.BatchNorm2d(d_decoder),
            )
    ...
    def forward(self, im, return_features=False, return_attention_stats=False):
        ...
        geom_features = im[:, :self.geometry_in_channels]

        if return_attention_stats:
            x, skip, attn_maps = self.encoder(
                im, return_features=True, return_attention_stats=True)
        else:
        ...
        need_decoder_features = (return_features or self.use_geometry_fusion) and hasattr(self.decoder, 'up_conv_block')
        if need_decoder_features:
            decoder_features = self.decoder(x, (H, W), skip, return_features=True)
            if self.use_geometry_fusion:
                geom_emb = self.geometry_proj(geom_features)
                fused = self.geometry_fusion(torch.cat((decoder_features, geom_emb), dim=1))
                decoder_features = decoder_features + fused
            feats = self.decoder.head(decoder_features)
        else:
        ...
        if decoder_features is not None:
            decoder_features = F.interpolate(
                decoder_features, size=(H, W), mode='bilinear', align_corners=False)
            decoder_features = unpadding(decoder_features, (H_ori, W_ori))

        if return_features or return_attention_stats:
            return {
                'logits': feats,
...
class RangeViT(nn.Module):
    def __init__(
        ...
        self.rangevit = create_rangevit(
            net_kwargs,
            use_kpconv=use_kpconv,
            use_geometry_fusion=use_geometry_fusion,
            geometry_fusion_channels=geometry_fusion_channels,
            geometry_in_channels=min(in_channels, 4),
        )
