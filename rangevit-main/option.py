...
class Option(object):
    def __init__(self, config_path, args):
        ...
        # The in-domain 80/20 frame split was removed: it drew train and val from the
        # same sequences, and at 10 Hz consecutive LiDAR frames are near-duplicates, so
        # validation scores were inflated by leakage. Fail loudly rather than silently
        # giving an old config a different meaning than it had when it was written.
        if 'use_in_domain_split' in self.config and self.config['use_in_domain_split']:
            raise ValueError(
                "'use_in_domain_split' has been removed (train/val leakage: it split "
                "frames within the same sequences). Use the standard protocol instead: "
                "train on sequences 00-07,09,10 and validate on 08. To reduce compute, "
                "subsample frames by stride across all training sequences rather than "
                "dropping sequences.")
        ...
        # Interpretable-by-design objective config
        self.use_interpretable_objective = self.config.get('use_interpretable_objective', False)
        self.prototype_use_geometry = self.config.get('prototype_use_geometry', True)
        self.prototype_geo_scale = self.config.get('prototype_geo_scale', 1.0)
        self.prototype_momentum = self.config.get('prototype_momentum', 0.9)
        self.prototype_margin = self.config.get('prototype_margin', 0.5)
        self.prototype_min_pixels = self.config.get('prototype_min_pixels', 128)
        self.lambda_prototype = self.config.get('lambda_prototype', 0.0)
        self.lambda_geometry = self.config.get('lambda_geometry', 0.0)
        self.lambda_attention_reg = self.config.get('lambda_attention_reg', 0.0)
        self.attention_entropy_target = self.config.get('attention_entropy_target', 0.45)
        self.attention_entropy_eps = self.config.get('attention_entropy_eps', 1e-6)

        # Geometry fusion architecture config
        self.use_geometry_fusion = self.config.get('use_geometry_fusion', False)
        self.geometry_fusion_channels = self.config.get('geometry_fusion_channels', 32)
