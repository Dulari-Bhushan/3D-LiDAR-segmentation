...
class Trainer(object):
    def __init__(self, settings: Option, model: nn.Module, recorder=None):
        ...
        # Interpretable objective state (lazy init at first training batch)
        self.prototype_bank = None
        self.prototype_valid = None
        if self.settings.use_interpretable_objective and self.settings.use_kpconv:
            if self.recorder is not None:
                self.recorder.logger.warning(
                    'Interpretable objective is currently implemented for no-KPConv training only. '
                    'Disabling interpretable objective for this run.')
            self.settings.use_interpretable_objective = False
    ...
    def _initDataloader(self):
        ...
        # SemanticKitti dataset
            ...
            # Standard SemanticKITTI protocol: sequences 00-07,09,10 train / 08 val.
            # The former in-domain 80/20 frame split drew train and val from the SAME
            # sequences; at 10 Hz consecutive frames are near-duplicates, so it leaked
            # heavily and its scores are not generalization numbers.
            test_sequences = (
    ...
    def compute_losses(self, output, output_softmax, label, mask):
        ...
        loss_lovasz = torch.nan_to_num(loss_lovasz, nan=0.0, posinf=1e4, neginf=0.0)
        loss_focal = torch.nan_to_num(loss_focal, nan=0.0, posinf=1e4, neginf=0.0)
    ...
    def _ensure_prototype_bank(self, joint_dim, device):
        if self.prototype_bank is not None:
            if self.prototype_bank.shape[1] != joint_dim:
                raise ValueError(
                    f'Prototype bank dimension mismatch: {self.prototype_bank.shape[1]} vs {joint_dim}')
            return
        self.prototype_bank = torch.zeros(
            self.settings.n_classes, joint_dim, device=device, dtype=torch.float32)
        self.prototype_valid = torch.zeros(
            self.settings.n_classes, device=device, dtype=torch.bool)

    def _compute_attention_objective(self, attention_maps):
        device = self.model.parameters().__next__().device
        zero = torch.zeros((), device=device)

        if not self.settings.use_interpretable_objective:
            return zero, zero
        if attention_maps is None or len(attention_maps) == 0:
            return zero, zero

        attn = attention_maps[-1]
        eps = self.settings.attention_entropy_eps
        n_tokens = attn.shape[-1]
        entropy = -(attn * torch.log(attn + eps)).sum(dim=-1)
        entropy = entropy / np.log(max(n_tokens, 2))
        entropy_mean = entropy.mean()
        entropy_target = torch.tensor(
            self.settings.attention_entropy_target, device=attn.device, dtype=attn.dtype)
        loss_attention = (entropy_mean - entropy_target).pow(2)
        return loss_attention, entropy_mean.detach()

    def _compute_prototype_objective(self, decoder_features, input_feature, input_label, input_mask):
        device = input_feature.device
        zero = torch.zeros((), device=device)

        if (not self.settings.use_interpretable_objective) or decoder_features is None:
            return zero, zero, zero

        labels = input_label.long()
        valid = (input_mask > 0) & (labels > 0)

        feat = decoder_features
        if feat.shape[-2:] != labels.shape[-2:]:
            feat = F.interpolate(feat, size=labels.shape[-2:], mode='bilinear', align_corners=False)

        feat = feat.permute(0, 2, 3, 1).contiguous().view(-1, feat.shape[1])
        labels_flat = labels.view(-1)
        valid_flat = valid.view(-1)

        if self.settings.prototype_use_geometry:
            geo = input_feature
            if geo.shape[-2:] != labels.shape[-2:]:
                geo = F.interpolate(geo, size=labels.shape[-2:], mode='bilinear', align_corners=False)
            geo = (geo * self.settings.prototype_geo_scale)
            geo = geo.permute(0, 2, 3, 1).contiguous().view(-1, geo.shape[1])
            joint = torch.cat([feat, geo], dim=1)
            geo_dim = geo.shape[1]
        else:
            joint = feat
            geo_dim = 0
        ...
        joint = joint[valid_flat]
        joint = torch.nan_to_num(joint.float(), nan=0.0, posinf=1e4, neginf=-1e4)
        labels_flat = labels_flat[valid_flat]

        self._ensure_prototype_bank(joint.shape[1], joint.device)
        ...
        with torch.no_grad():
            for c in unique_classes:
                ...
                if c_int <= 0:
                    continue
                cls_mask = (labels_flat == c_int)
                if cls_mask.sum().item() < self.settings.prototype_min_pixels:
                    continue
                cls_mean = joint[cls_mask].mean(dim=0).detach()
                if self.prototype_valid[c_int]:
                    m = self.settings.prototype_momentum
                    self.prototype_bank[c_int] = m * self.prototype_bank[c_int] + (1.0 - m) * cls_mean
                else:
                    self.prototype_bank[c_int] = cls_mean
                    self.prototype_valid[c_int] = True

        valid_proto_for_label = self.prototype_valid[labels_flat]
        if valid_proto_for_label.sum().item() == 0:
            return zero, zero, zero

        joint = joint[valid_proto_for_label]
        labels_flat = labels_flat[valid_proto_for_label]

        dists = torch.cdist(joint.float(), self.prototype_bank.float(), p=2)
        dists = torch.nan_to_num(dists, nan=1e6, posinf=1e6, neginf=1e6)
        row_idx = torch.arange(dists.shape[0], device=dists.device)
        true_dists = dists[row_idx, labels_flat]
        loss_compact = (true_dists * true_dists).mean()

        wrong_dists = dists.clone()
        wrong_dists[row_idx, labels_flat] = 1e6
        min_wrong = wrong_dists.min(dim=1).values
        loss_sep = F.relu(self.settings.prototype_margin + true_dists - min_wrong).mean()
        loss_proto = loss_compact + loss_sep
        loss_proto = torch.nan_to_num(loss_proto, nan=0.0, posinf=1e4, neginf=0.0)

        nearest = dists.argmin(dim=1)
        proto_purity = (nearest == labels_flat).float().mean().detach()
        proto_purity = torch.nan_to_num(proto_purity, nan=0.0, posinf=1.0, neginf=0.0)

        loss_geo = zero
        if self.settings.prototype_use_geometry and geo_dim > 0:
            geo_feats = joint[:, -geo_dim:]
            class_vars = []
            for c in labels_flat.unique():
                cls_mask = (labels_flat == c)
                if cls_mask.sum().item() > 1:
                    class_vars.append(geo_feats[cls_mask].var(dim=0, unbiased=False).mean())
            if len(class_vars) > 0:
                loss_geo = torch.stack(class_vars).mean()
                loss_geo = torch.nan_to_num(loss_geo, nan=0.0, posinf=1e4, neginf=0.0)

        return loss_proto, loss_geo, proto_purity
    ...
    def run_without_kpconv(self, epoch, mode='Train', print_results=False, save_results_path=None):
        ...
        loss_proto_meter = tools.AverageMeter()
        loss_geo_meter = tools.AverageMeter()
        loss_attn_meter = tools.AverageMeter()
        proto_purity_meter = tools.AverageMeter()
        attn_entropy_meter = tools.AverageMeter()
        self.metrics.reset()
        ...
        for i, (input_feature, input_label, input_mask) in enumerate(dataloader):
            ...
            if mode == 'Train':
                with torch.cuda.amp.autocast(self.fp16_scaler is not None):
                    ...
                    attention_maps = None
                    if self.settings.use_interpretable_objective:
                        output_dict = self.model(
                            input_feature,
                            return_features=True,
                            return_attention_stats=True)
                        output = output_dict['logits']
                        decoder_features = output_dict['decoder_features']
                        attention_maps = output_dict['attention_maps']
                    else:
                        output = self.model(input_feature)
                    output = torch.nan_to_num(output, nan=0.0, posinf=20.0, neginf=-20.0)
                    output = torch.clamp(output, min=-20.0, max=20.0)
                    output_softmax = F.softmax(output, dim=1).clamp(min=1e-6, max=1.0 - 1e-6)
                    ...
                    if self.settings.use_interpretable_objective:
                        loss_proto, loss_geo, proto_purity = self._compute_prototype_objective(
                            decoder_features, input_feature, input_label, input_mask)
                        loss_attn, attn_entropy = self._compute_attention_objective(attention_maps)

                        loss_proto = torch.nan_to_num(loss_proto, nan=0.0, posinf=1e4, neginf=0.0)
                        loss_geo = torch.nan_to_num(loss_geo, nan=0.0, posinf=1e4, neginf=0.0)
                        loss_attn = torch.nan_to_num(loss_attn, nan=0.0, posinf=1e4, neginf=0.0)
                        proto_purity = torch.nan_to_num(proto_purity, nan=0.0, posinf=1.0, neginf=0.0)
                        attn_entropy = torch.nan_to_num(attn_entropy, nan=0.0, posinf=1.0, neginf=0.0)

                        total_loss = (
                            total_loss
                            + self.settings.lambda_prototype * loss_proto
                            + self.settings.lambda_geometry * loss_geo
                            + self.settings.lambda_attention_reg * loss_attn)
                        total_loss = torch.nan_to_num(total_loss, nan=0.0, posinf=1e4, neginf=0.0)
                    else:
                        loss_proto = torch.zeros((), device=input_feature.device)
                        loss_geo = torch.zeros((), device=input_feature.device)
                        loss_attn = torch.zeros((), device=input_feature.device)
                        proto_purity = torch.zeros((), device=input_feature.device)
                        attn_entropy = torch.zeros((), device=input_feature.device)

                # Backward
                self.optimizer.zero_grad()
                loss_for_backward = torch.nan_to_num(total_loss.mean(), nan=0.0, posinf=1e4, neginf=0.0)

                # Keep all DDP ranks in sync: if any rank has non-finite loss, skip the step on all ranks.
                finite_flag = torch.isfinite(loss_for_backward.detach())
                if tools.is_dist_avail_and_initialized():
                    finite_tensor = finite_flag.to(dtype=torch.int32)
                    torch.distributed.all_reduce(finite_tensor, op=torch.distributed.ReduceOp.MIN)
                    is_finite_all_ranks = (finite_tensor.item() == 1)
                else:
                    is_finite_all_ranks = bool(finite_flag.item())

                if not is_finite_all_ranks:
                    if self.recorder is not None and tools.is_main_process():
                        self.recorder.logger.warning(
                            f'Skip non-finite loss at epoch {epoch + 1} iter {i + 1} '
                            f'(loss={loss_for_backward.item()})')
                    continue
                ...
                if self.fp16_scaler is None:
                    ...
                    self.fp16_scaler.scale(loss_for_backward).backward()
            ...
            loss = torch.nan_to_num(total_loss.mean(), nan=0.0, posinf=1e4, neginf=0.0)
            with torch.no_grad():
                argmax = output.argmax(dim=1)
                self.metrics.addBatch(argmax, input_label) # 2D predictions

            loss_meter.update(loss.item(), input_feature.size(0))
            if self.settings.use_interpretable_objective and mode == 'Train':
                loss_proto_meter.update(loss_proto.detach().item(), input_feature.size(0))
                loss_geo_meter.update(loss_geo.detach().item(), input_feature.size(0))
                loss_attn_meter.update(loss_attn.detach().item(), input_feature.size(0))
                proto_purity_meter.update(proto_purity.detach().item(), input_feature.size(0))
                attn_entropy_meter.update(attn_entropy.detach().item(), input_feature.size(0))
            ...
            if (i % self.settings.log_frequency == 0) or (i == total_iter-1):
                with torch.no_grad():
                    # local=True: progress logging only, never a cross-rank collective
                    mean_iou, _, mean_acc, _ = self.metrics.getIoUnAcc(local=True)
                if self.recorder is not None:
                    ...
                    if self.settings.use_interpretable_objective and mode == 'Train':
                        log_str += (
                            'LossProto {:0.4f} LossGeo {:0.4f} LossAttn {:0.4f} '
                            'ProtoPurity {:0.4f} AttnEntropy {:0.4f} '
                        ).format(
                            loss_proto.detach().item(),
                            loss_geo.detach().item(),
                            loss_attn.detach().item(),
                            proto_purity.detach().item(),
                            attn_entropy.detach().item())
                    log_str += 'RT {}'.format(remain_time)
                    self.recorder.logger.info(log_str)

        with torch.no_grad():
            # Single cross-rank reduction for the whole epoch. Must be reached by
            # every rank, so keep it outside any is_main_process() guard.
            ...
            if self.settings.use_interpretable_objective and mode == 'Train':
                metrics_dict['proto_purity'] = torch.tensor(proto_purity_meter.avg)
                metrics_dict['attention_entropy'] = torch.tensor(attn_entropy_meter.avg)
        ...
        if self.settings.use_interpretable_objective and mode == 'Train':
            loss_dict['loss_prototype'] = torch.tensor(loss_proto_meter.avg)
            loss_dict['loss_geometry'] = torch.tensor(loss_geo_meter.avg)
            loss_dict['loss_attention_reg'] = torch.tensor(loss_attn_meter.avg)
        ...
        for i, batch_dict in enumerate(dataloader):
            ...
            if mode == 'Train':
                with torch.cuda.amp.autocast(self.fp16_scaler is not None):
                    ...
                    output3d = torch.nan_to_num(output3d, nan=0.0, posinf=20.0, neginf=-20.0)
                    output3d = torch.clamp(output3d, min=-20.0, max=20.0)
                    output3d_softmax = F.softmax(output3d, dim=1).clamp(min=1e-6, max=1.0 - 1e-6)
                ...
                # Backward
                self.optimizer.zero_grad()
                loss_for_backward = total_loss.mean()
