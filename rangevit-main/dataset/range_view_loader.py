...
class RangeViewLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        pointcloud, sem_label, inst_label = self.dataset.loadDataByIndex(index)
        # Must match the guard in get_item_for_kpconv(). ScanProjection recovers the
        # scanline row by detecting azimuth wrap-around in the raw firing order, so any
        # 3D rotation/translation of the cloud injects spurious wraps and scrambles the
        # vertical axis (overflow rows get clamped onto the last row). Augmentation is
        # therefore only valid under the FOV-based RangeProjection.
        if self.is_train and (self.scan_proj is False):
            pointcloud = self.augmentor.doAugmentation(pointcloud)  # n, 4
        proj_pointcloud, proj_range, proj_idx, proj_mask = self.projection.doProjection(pointcloud)
