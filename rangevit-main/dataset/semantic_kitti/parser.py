    ...
    def __init__(self, root,  # directory where data is
        ...
        if os.path.isdir(self.root):
            ...
            raise ValueError(
                ...
                f'--data_root must point at the directory containing the numbered '
                f'sequence folders, i.e. <semantic_kitti>/dataset/sequences/ , '
                f'which holds 00/, 01/, ... each with velodyne/ and labels/.')
        ...
        for seq in self.sequences:
            ...
            if not os.path.isdir(pointcloud_path):
                raise FileNotFoundError(
                    f'Missing velodyne directory for sequence {seq}: {pointcloud_path}\n'
                    f'Expected layout: {self.root}/{seq}/velodyne/*.bin '
                    f'and {self.root}/{seq}/labels/*.label')
            pointcloud_files = [
            ...
            if self.has_label:
                ...
                if not os.path.isdir(label_path):
                    raise FileNotFoundError(
                        f'Missing labels directory for sequence {seq}: {label_path}\n'
                        f'Sequences 11-21 are unlabelled; use --test_split for those.')
                label_files = [
